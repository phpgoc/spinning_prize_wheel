...
!macro NSIS_HOOK_POSTINSTALL
  ${If} $NoShortcutMode <> 1
    ...
    ; 两个版本都直接创建桌面入口，避免 Tauri 默认逻辑只处理主程序。
    CreateShortcut "$DESKTOP\转盘.lnk" "$INSTDIR\转盘.exe"
    CreateShortcut "$DESKTOP\转盘-猜蜜版.lnk" "$INSTDIR\转盘-猜蜜版.exe"
  ${EndIf}
!macroend
